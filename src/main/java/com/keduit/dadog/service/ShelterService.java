...
public class ShelterService {
    ...
    public void fetchAndSaveShelterData() {
        // 데이터베이스에 보호소 정보가 있는지 확인
        List<Shelter> existingShelters = shelterRepository.findAll();
        if (!existingShelters.isEmpty()) {
            logger.info("보호소 데이터가 이미 저장되어 있습니다. API 호출을 생략합니다.");
            return; // 이미 저장된 경우 API 호출 생략
        }
